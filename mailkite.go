...
// CreateAppPasswordRequest is the body of CreateAppPassword(): Domain is
// required; Address defaults to "*" (every address on the domain) and
// Protocols defaults to ["imap"].
type CreateAppPasswordRequest struct {
	// The hosted domain this password covers. Must belong to the calling
	// account.
	Domain string `json:"domain"`
	// Local-part pattern within the domain — "*", "hello", "support-*",
	// "*-agent". No "@": the domain is already named.
	Address string `json:"address,omitempty"`
	// What the password may authenticate: "imap", "api", or both.
	Protocols []string `json:"protocols,omitempty"`
	// Optional name shown in the dashboard.
	Label string `json:"label,omitempty"`
}

// ListAppPasswords lists the account's app passwords. Each one opens a
// mailbox over IMAP and/or the mailbox API, scoped to a domain and an
// address pattern within it.
func (c *Client) ListAppPasswords() (interface{}, error) {
	return c.Request("GET", "/api/app-passwords", nil)
}

// CreateAppPassword creates an app password for one domain and address
// pattern. Hand it to a mail client or an agent — the secret is returned
// once and never again.
func (c *Client) CreateAppPassword(body interface{}) (interface{}, error) {
	return c.Request("POST", "/api/app-passwords", body)
}

// DeleteAppPassword revokes an app password. Takes effect immediately — any
// IMAP session or API call using it stops authenticating.
func (c *Client) DeleteAppPassword(id string) (interface{}, error) {
	return c.Request("DELETE", "/api/app-passwords/"+url.PathEscape(id), nil)
}

// ListMailboxMessages lists a mailbox's messages, newest first. Authenticated
// with an app password granting "api" — this is how an agent reads its own
// inbox without an IMAP client. params may carry "limit" and "before".
func (c *Client) ListMailboxMessages(address string, params map[string]string) (interface{}, error) {
	q := url.Values{"address": {address}}
	for k, v := range params {
		q.Set(k, v)
	}
	return c.Request("GET", "/api/mailbox/messages?"+q.Encode(), nil)
}

// GetMailboxMessageRaw fetches one message's raw RFC822 bytes from a mailbox.
// Same app password auth as the list.
func (c *Client) GetMailboxMessageRaw(uid int, address string) (interface{}, error) {
	q := url.Values{"address": {address}}
	return c.Request("GET", fmt.Sprintf("/api/mailbox/messages/%d/raw?%s", uid, q.Encode()), nil)
}

// SetMailboxMessageFlags replaces a message's IMAP flags (e.g. mark it
// "Seen"). Flags set here are the same ones an IMAP client sees.
func (c *Client) SetMailboxMessageFlags(uid int, address, flags string) (interface{}, error) {
	q := url.Values{"address": {address}}
	return c.Request("POST", fmt.Sprintf("/api/mailbox/messages/%d/flags?%s", uid, q.Encode()), map[string]string{"flags": flags})
}
